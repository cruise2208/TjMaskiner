...
class WC_QuickPay_Subscription {
    ...
    */
    public static function process_recurring_response( $recurring_response, $order )
    {
	    ...
	    $order->update_meta_data('_quickpay_transaction_id', $recurring_response->id);
	    $order->save_meta_data();

        $autocomplete_renewal_orders = WC_QP()->s('subscription_autocomplete_renewal_orders');

        if (WC_QuickPay_Helper::option_is_enabled($autocomplete_renewal_orders)) {
        	$order->update_status( 'completed', __( 'Automatically completing order status due to succesful recurring payment', 'woo-quickpay' ), false );
        }
    }
