...
function init_quickpay_gateway() {
	...
	require_once WCQP_PATH . 'classes/modules/woocommerce-quickpay-checkout-frontend.php';
	require_once WCQP_PATH . 'classes/modules/woocommerce-quickpay-checkout.php';
	...
	class WC_QuickPay extends WC_Payment_Gateway {
		...
		public function hooks_and_filters() {
			...
			WC_QuickPay_Checkout_Frontend::get_instance();
			WC_QuickPay_Checkout::get_instance();
					...
					if ( $payment->is_action_allowed( $param_action ) ) {
						...
						if ( method_exists( $payment, $param_action ) ) {
							...
							call_user_func_array( [ $payment, $param_action ], [
								$transaction_id,
								$order,
								WC_QuickPay_Helper::price_multiplied_to_float( $amount ),
							] );
						} else {
					...
					if ( $transaction_id ) {
						try {
							...
							if ( $payment->is_action_allowed( 'capture' ) ) {
								...
								$amount_multiplied = WC_QuickPay_Helper::price_multiply( $order->get_total() ) - $payment->get_balance();
								$amount            = WC_QuickPay_Helper::price_multiplied_to_float( $amount_multiplied );
		...
		public function receipt_page( $order_id ) {
			if ( WC_QuickPay_Checkout_Frontend::is_embedded_payment_enabled() ) {
				wp_enqueue_script( 'wcqp-overlay' );
				echo WC_QuickPay_Checkout_Frontend::generate_overlay_form_for_order( $order_id );
			}
		}
		...
		public function process_payment( $order_id ) {
			...
			if ( 'quickpay' === $this->id && WC_QuickPay_Helper::option_is_enabled( WC_QP()->s( 'quickpay_embedded_payments_enabled' ) ) ) {

				$redirect = $order->get_checkout_payment_url( true );
				if ( ! empty( $_REQUEST['woocommerce_change_payment'] ) ) {
					$redirect = add_query_arg( 'quickpay_change_payment_method', $_REQUEST['woocommerce_change_payment'], $redirect );
				}
				...
				try {
					if ( $transaction_id && $order->has_quickpay_payment() ) {
						...
						WC_QuickPay_Views::get_view( 'html-order-table-transaction-data.php', [
							...
							'transaction_brand_logo_url' => WC_QuickPay_Helper::get_payment_type_logo( $transaction->get_brand() ),
